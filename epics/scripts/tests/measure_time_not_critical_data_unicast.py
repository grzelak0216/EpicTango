...
def get_pv_read(module, device_list, data_type):
    ...
    for i in device_list:
        results.append(f"{module}:{infix}{i}:notCritical:{postfix}:received")
    return results
...
def get_pv_write(module, device_list, data_type):
    ...
    results=[]
    for i in device_list:
        results.append(f"{module}:{infix}{i}:notCritical:{postfix}")
    return results
...
def compute_and_save_stats(time_diffs, unicast_time, pvs_name_read_list, module, device_number, number_of_read, file_num,
    ...
    if module == OPCUA:
        filename=f"./results/multi_devices/not_critical/{module}_{infix}_{device_number.replace(':', '_')}_{test_type.lower()}_{number_of_read}_{min_sampling_interval}_{min_publishing_interval}"
    else:
        filename=f"./results/multi_devices/not_critical/{module}_{infix}_{device_number.replace(':', '_')}_{test_type.lower()}_{number_of_read}"
...
def run_analog(module, device_list, number_of_read, test_type, device_number, data_type, 
        print_result_dict, save_to_file, from_docker, file_num, min_sampling_interval=None, min_publishing_interval=None):
    times_array = []
    unicast_time =[]
    ...
    for j in range(len(pvs_name_write_list)):
        print(caget(pvs_name_write_list[j]))
        time.sleep(0.1)
        # print(pvs_name_write_list[j])
    ...
    while len(times_array) < number_of_read:
        ...
        for i in range(0,2):
            start = time.time()
            pv_val = random.randint(0, 2**31 - 1)
            for j in range(len(pvs_name_write_list)):
...
if __name__ == "__main__":
    if len(sys.argv) < 9:
        ...
        print("<data type> = single / analog (s/a)")
    ...
    if module == OPCUA:
        ...
        if data_type.lower() == "s":
            ...
            run_analog(module, device_list, number_of_read, test_type, device_number, data_type, 
                print_result_dict=True, save_to_file=save_to_file, from_docker=from_docker, file_num=file_num,
                min_sampling_interval=min_sampling_interval, min_publishing_interval=min_publishing_interval)
    else:
        if data_type.lower() == "s":
